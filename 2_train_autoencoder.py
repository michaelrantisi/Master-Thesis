...
class Tiles6ChDataset(torch.utils.data.Dataset):
...
def main():
    ...
    df = df[df["label"] == "healthy"].copy()
    if len(df) == 0:
        raise RuntimeError("No healthy tiles in manifest.")

    tr_df, va_df = split_by_source(df, test_size=0.2, seed=42)

    train_ds = Tiles6ChDataset(tr_df)
    val_ds   = Tiles6ChDataset(va_df)

    train_loader = DataLoader(train_ds, batch_size=32, shuffle=True, num_workers=0, pin_memory=True)
    val_loader   = DataLoader(val_ds, batch_size=32, shuffle=False, num_workers=0, pin_memory=True)

    model = SmallConvAE(in_ch=6).to(device)
    ...
    for epoch in range(1, 31):
        model.train()
        tr = 0.0
        for x in tqdm(train_loader, desc=f"Train AE {epoch}"):
        ...
        with torch.no_grad():
            for x in val_loader:
                x = x.to(device, non_blocking=True)
                recon = model(x)
                va += crit(recon, x).item()
        va /= max(1, len(val_loader))

        print(f"Epoch {epoch}: train={tr:.6f} val={va:.6f}")
        if va < best:
            best = va
            torch.save(model.state_dict(), os.path.join(OUT, "ae_best_6ch.pt"))
            print("Saved best AE 6ch.")
